...
int zui::DIN;
int zui::CLK;
int zui::cursor_pos=0;//记录cpu执行到那个坐标
...
void zui::ButtonVar(int x, int y, double number, int b, boolean float_number = 0) //光标标注,注册选项,保存各个光标的坐标
{
	...
	{
		if (cursor_pos==cursor_now)//符合当前光标坐标的反显输出，不符合的正常输出
		{
...
//数据更改对话框   number被修改的数据，b要打印出几位数，是否为浮点，每摇杆延时时间的步进程度
...
void zui::ArrowEnd()
{
	if (!page_arrow||ui.PageSwitch())//从home键回来时页面已经同步但光标样式没变，第一次绘图会有残留，所以需要两个
	{
		Clear();
	}
	...
	first_record = 0;
	cursor_pos = 0;
	if (sync)//在页面跳转后，在end()处不进行同步，这样才能知道页面是否有切换
	{
		PageSync();
	}
	else
