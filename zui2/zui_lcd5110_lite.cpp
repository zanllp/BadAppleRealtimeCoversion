...
int ZuiLite::DIN;
int ZuiLite::CLK;
const char ZuiLite::decimal_point[]PROGMEM = { 0x00, 0x40, }; //小数点
const char ZuiLite::ASCIIDZ[67] = " !:abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.";
char ZuiLite::char_conver_temp[112];//字符转换缓存
const char ZuiLite::xz[8] PROGMEM = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, };//用到的地方不多就不重写名字了
const char ZuiLite::ASCII[92][6]PROGMEM = {
...
//LCD初始化函数
void ZuiLite::SetLcd(int RST0, int CE0, int DC0, int DIN0, int CLK0)
{
	RST = RST0;
	CE = CE0;
	DC = DC0;
...
//设置字符位置函数  xy坐标
void ZuiLite::SetPostion(unsigned char X, unsigned char Y)
{
	WriteData(0x40 | Y, 0);// column
	WriteData(0x80 | X, 0);// row
}
//数据写入到lcd dat数据，command 0命令1数据选择
void ZuiLite::WriteData(unsigned char dat, unsigned char command)//data ：写入的数据；command ：写数据 / 命令选择；
{
	unsigned char i;
...
//点          xy坐标
void ZuiLite::point(int x, int y)//点函数
{
	SetPostion(x, int(y / 8));
	WriteData(pgm_read_byte(&xz[int(y % 8)]), 1);
}
//画图        xy坐标(0-83,0-5)，width宽度,length长度,l点阵数据,是否反显，是否从内存读取数据
void ZuiLite::Draw(int x, int y, int width, int length, const char *l, bool reverse_display = 0, bool from_memory = 0)//xy图片左上角点的x（0-83）y（0-5）值，width为图标的宽度，length为图标的长度,l为点阵数据
{ //从flash中读取数据
	int i;
	int ii;
	int ys = length / 8;
...
void ZuiLite::Var(int x, int y, double number, int b, boolean  float_number = 0, bool reverse_display = 0)
{
...
void ZuiLite::Text(int x, int y, char* l, boolean reverse_display = 0)
{
	String txtMsg = l;
